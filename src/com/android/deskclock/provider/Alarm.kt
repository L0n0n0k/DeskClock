...
class Alarm : Parcelable, AlarmsColumns {
    ...
    companion object {
         ...
         */
        private val DEFAULT_SORT_ORDER = ClockDatabaseHelper.ALARMS_TABLE_NAME + "." + AlarmsColumns.HOUR + ", " +
                ClockDatabaseHelper.ALARMS_TABLE_NAME + "." + AlarmsColumns.MINUTES + " ASC" + ", " +
         ...
         */
        @JvmStatic
        fun getAlarms(cr: ContentResolver, selection: String?,
                      vararg selectionArgs: String?): List<Alarm> {
            val result: MutableList<Alarm> = LinkedList()
            val cursor: Cursor? = cr.query(AlarmsColumns.CONTENT_URI, QUERY_COLUMNS, selection, selectionArgs, null)
            cursor?.let {
                if (cursor.moveToFirst()) {
                    do {
        ...
        fun updateAlarm(contentResolver: ContentResolver, alarm: Alarm): Boolean {
            ...
            val values: ContentValues = createContentValues(alarm)
            val rowsUpdated: Long = contentResolver.update(getContentUri(alarm.id), values, null, null).toLong()
