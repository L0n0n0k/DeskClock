...
class AlarmInstance : InstancesColumns {
    ...
    companion object {
        ...
        fun getInstance(cr: ContentResolver, instanceId: Long): AlarmInstance? {
            val cursor: Cursor? = cr.query(getContentUri(instanceId), QUERY_COLUMNS, null, null, null)
            cursor?.let {
         ...
         */
        @JvmStatic
        fun getInstancesByAlarmId(contentResolver: ContentResolver,
                                  alarmId: Long): List<AlarmInstance> {
         ...
         */
        @JvmStatic
        fun getNextUpcomingInstanceByAlarmId(contentResolver: ContentResolver,
                                             alarmId: Long): AlarmInstance? {
        ...
        fun getInstancesByInstanceIdAndState(
                contentResolver: ContentResolver, alarmInstanceId: Long, state: Int): List<AlarmInstance> {
            return getInstances(contentResolver, _ID.toString() + "=" + alarmInstanceId + " AND " + InstancesColumns.ALARM_STATE +
                    "=" + state)
        }
        ...
        fun getInstancesByState(
                contentResolver: ContentResolver, state: Int): List<AlarmInstance> {
            return getInstances(contentResolver, InstancesColumns.ALARM_STATE + "=" + state)
        }
         ...
         */
        @JvmStatic
        fun getInstances(cr: ContentResolver, selection: String?,
                         vararg selectionArgs: String?): List<AlarmInstance> {
            val result: MutableList<AlarmInstance> = LinkedList()
            val cursor: Cursor? = cr.query(InstancesColumns.CONTENT_URI, QUERY_COLUMNS, selection, selectionArgs, null)
            cursor?.let {
                if (cursor.moveToFirst()) {
                    do {
        ...
        @JvmStatic
        fun addInstance(contentResolver: ContentResolver,
                        instance: AlarmInstance): AlarmInstance {
        ...
        fun updateInstance(contentResolver: ContentResolver, instance: AlarmInstance): Boolean {
            ...
            val values: ContentValues = createContentValues(instance)
            val rowsUpdated: Long = contentResolver.update(getContentUri(instance.mId), values, null, null).toLong()
        ...
        @JvmStatic
        fun deleteOtherInstances(context: Context?, contentResolver: ContentResolver,
